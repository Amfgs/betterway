...
import { Check, ChevronRight, Clock3, Plus, Search, Target, Trash2, UserPlus, Users, WalletCards, X } from "lucide-react";
...
export function FriendsPage() {
  ...
  async function loadWorkspace() {
    const [friendsResponse, goalsResponse, limitsResponse] = await Promise.all([
      api.get("/friends"),
      api.get("/goals"),
      api.get("/limits")
    ]);
  ...
  async function createSharedGoal(event) {
    ...
    setWorking("goal");
    try {
      await api.post("/goals", { ...sharedGoalForm, participantIds: [selectedFriend.id] });
  ...
  async function createSharedLimit(event) {
    ...
    setWorking("limit");
    try {
      await api.post("/limits", { ...sharedLimitForm, participantIds: [selectedFriend.id] });
            ...
            {friends.map((friend) => {
              ...
              return (
                ...
                >
                  ...
                  <ChevronRight className={active ? "text-emerald-600" : "text-zinc-400"} size={18} />
                </button>
              );
            })}
          </div>
            ...
            <section className="rounded-lg border border-black/5 bg-white p-4 shadow-soft dark:border-white/10 dark:bg-neutral-900 sm:p-5">
              ...
              {planMode === "goal" ? (
                <form className="mt-4 grid gap-3" onSubmit={createSharedGoal}>
                  ...
                  <button className="rounded-lg bg-emerald-500 px-4 py-3 font-black text-white disabled:opacity-50" disabled={Boolean(working)} type="submit">{working === "goal" ? "Criando..." : `Criar meta com ${selectedFriend.name.split(" ")[0]}`}</button>
                </form>
              ) : (
                <form className="mt-4 grid gap-3" onSubmit={createSharedLimit}>
                  ...
                  <button className="rounded-lg bg-emerald-500 px-4 py-3 font-black text-white disabled:opacity-50" disabled={Boolean(working)} type="submit">{working === "limit" ? "Criando..." : `Criar limite com ${selectedFriend.name.split(" ")[0]}`}</button>
                </form>
              )}
            </section>
          ) : (
